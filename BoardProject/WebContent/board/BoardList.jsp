...
<body>
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
		...
		<tr height="25" align="center">
			<td>&nbsp;</td>
			<td>${b.num}</td>
			<td align="left"><a href="Board.do?action=view&num=${b.num}">${b.title}</a></td>
			<td align="center">${b.writer}</td>
			<td align="center">${b.regdate}</td>
			<td align="center">${b.rate+1}</td>
			<td>&nbsp;</td>
		</tr>
        ...
        <c:if test="${pageIsPrev}">
            <a href="Board.do?action=list&page=${pageStart - 1}">prev</a>
        </c:if>
        <c:forEach var="page" begin="${pageStart}" end="${pageEnd}">
            <a href="Board.do?action=list&page=${page}">[${page}] </a>
        </c:forEach>
        <c:if test="${pageIsNext}">
            <a href="Board.do?action=list&page=${pageEnd + 1}">next</a>
        </c:if>
    </center>
	...
	게시물 갯수 : ${resultList.size()} <br>
	<a href="BoardInsert.jsp">게시물 등록</a>
